...
app = FastAPI(title="Parquet to CSV Converter", version="1.0.0")
...
async def save_upload_to_disk(upload: UploadFile, destination: Path) -> None:
    bytes_written = 0
...
async def convert(background_tasks: BackgroundTasks, file: UploadFile = File(...)):
    ...
    temp_dir = tempfile.mkdtemp(prefix="parquet-to-csv-")
    input_path = Path(temp_dir) / "input.parquet"
    output_path = Path(temp_dir) / "output.csv"

    try:
        await save_upload_to_disk(file, input_path)
        await asyncio.to_thread(convert_parquet_to_csv, input_path, output_path)
